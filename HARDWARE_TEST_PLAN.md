...
1. **A physical trigger sensor** — C3 loopback is now closed (§7); C4 is not.
2. **A valid device token on the Pi** — the backend and Postgres are up, but
   `raspberry_pi_code/.env` carries a `DEVICE_TOKEN` the current database does
   not recognise, so every upload 401s.

*(Real model weights were the biggest gap and are now closed — see §8.)*
...
## 8. Real model weights — 2026-07-25 (branch `phase-4-real-weights`)

Both tiers moved off their stand-ins. Until now **accuracy had never been
measured** on this project, because there was nothing meaningful to measure:
Tier 1 ran a generated smoke-test model and Tier 2 a randomly-initialised head.

### Results

| Tier | Model | Top-1 | Latency |
|---|---|---|---|
| **1** (Pi 5) | Google AIY `birds_V1` — MobileNetV2/iNaturalist, 964 spp + `background` | **20/20** | **57.9 ms** avg (min 55.5 / max 60.3), 12 ms load |
| **2** (RTX 5080) | `hf-hub:timm/vit_large_patch14_clip_336.laion2b_ft_augreg_inat21` — 10,000 spp | **20/20** | **29.1 ms** avg (min 24.8 / max 53.0), ~26 s cold load |

Confidences ran 0.85–0.97 on both tiers, with one outlier — Ruby-throated
Hummingbird at 0.578 (Tier 1) / 0.601 (Tier 2). Every prediction clears the
`confidence_threshold=0.5` gate, so the tier chain now *accepts* Tier 1 answers
instead of always falling through to Tier 2 and Tier 3 as it did with the
stand-in (which scored 0.05–0.07 on everything).

> **The 20/20 is an upper bound.** Ground-truth photos are Wikipedia lead images:
> clean, centred, well-lit, frame-filling birds. A feeder camera sees motion
> blur, bad angles, backlighting and partial occlusion. This measures that the
> model and taxonomy are correct, not that the system is accurate in the field.

### How the label spaces were reconciled

`taxonomy.csv` went from 20 curated rows to **965 — the model's own label
space**, generated by `scripts/build_taxonomy.py`. Row order is the index
contract (`tier1_tflite.py` does `self._taxa[argmax]`), so the file is generated
and must never be hand-edited.

Tier 2's model has a *different* label space (10,000 iNat21 classes). It
publishes `label_names` in its Hub `config.json`, so `_build_projection` maps
each output index onto the taxonomy by scientific name — **869 of 965 map**, and
20/20 of the curated feeder species. Softmax runs over all 10,000 classes and
the best *mapped* class is then taken, so a squirrel leaves every bird class low
and falls through the confidence gate rather than being renormalised into a
confident bird.

### Findings

12. **A taxonomy/model mismatch is invisible to every other check.** Shapes,
    latency and load all pass with a taxonomy that is off by one row, while
    every sighting gets a confidently wrong species.
    `scripts/validate_tier1.py` exists for this: a misaligned taxonomy scores
    near zero and the script exits non-zero. Run it after any model change.
13. **The model's naming is frozen at ~2023 iNaturalist taxonomy.** Downy
    Woodpecker is `Picoides pubescens` there and `Dryobates pubescens` today, so
    a naive name match dropped it. `CURRENT_NAMES` in `build_taxonomy.py`
    records such renames — the CSV shows today's name, the row stays at the
    index the model emits.
14. **Wikipedia prefers IOC names over AOS ones.** Left alone it renames
    `European Starling` to `Common Starling` across the app and the seed data.
    The curated `feeder_species.csv` names now win wherever the two overlap.
15. **Expanding `taxonomy.csv` would have broken the demo.** The simulator drew
    its species from it, and `species_weights()` makes index 0 the commonest
    feeder bird — so the dashboard would have filled with Cassin's Finches,
    limpkins and cranes. The simulator now reads `feeder_species.csv`, a curated
    subset, which keeps the "every simulated bird is one Tier 1 could predict"
    property while staying plausible.

### Notes

- Weights are **fetched, not committed** (`scripts/fetch_models.py`, sha256-pinned).
- Both the Kaggle model download and the Wikipedia label lookups are anonymous —
  no account or API key is needed.
- Wikipedia rate-limits bursts: both `build_taxonomy.py` and `validate_tier1.py`
  back off on 429. `build_taxonomy.py` refuses to write a partial file rather
  than silently emitting a taxonomy with gaps.
- **The inference server must be restarted** to pick up the new model; it caches
  the classifier at lifespan startup.

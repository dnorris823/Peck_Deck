...
class UserOperations:
    ...
    async def create_users(self, req_data: UsersCreatorRequestSchema, db_connection: async_sessionmaker) -> UsersResponseSchema:
    ...
    async def update_users(self, req_data: UsersUpdaterRequestSchema, db_connection: async_sessionmaker) -> UsersResponseSchema:

        req_data = req_data.model_dump()
        print(f"REQ DATA UPDATE: {req_data}")
        async with db_connection() as session:
            
            updated_users_list = []
            
            for record in req_data.get('records_list'):
                updated_users = await session.execute(select(Users).where(Users.id == record.get("user_id")))
                updated_users = updated_users.scalar_one()

                # go through all optional items in request and update users if present
                for k, v in record.items():
                    if k != 'users_id' and v is not None:
                        if k == 'password':
    ...
    async def delete_users(self, req_data: UsersDeleterRequestSchema, db_connection: async_sessionmaker) -> None:
        ...
        async with db_connection() as session:
            ...
            for record in req_data.get('records_list'):
                ...
                users = await session.execute(select(Users).where(Users.id == record.get("user_id")))
